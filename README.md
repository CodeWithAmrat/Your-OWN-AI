...
5. Add g++ to your Windows PATH:
   ...
   g++ --version
   ```
    You should see something like `g++ (GCC) 15.x.x` from MSYS2 UCRT64.

    If you see `C:\MinGW\bin\g++.exe` or an old `MinGW.org GCC 6.x` build, do not use it for this project. That compiler does not provide the C++ thread support required by `httplib.h`.
...
```powershell
git clone https://github.com/CodeWithAmrat/Your-OWN-AI.git
cd Your-OWN-AI
```

There is no separate repository to create for this project. Clone the repository you were given, then build and run it locally.

---

### Step 5 — Compile the C++ Server

Inside the cloned project folder, run this from a fresh **MSYS2 UCRT64 shell** or from PowerShell after adding `C:\msys64\ucrt64\bin` to `PATH` and setting `MSYSTEM=UCRT64`:

```powershell
g++ -std=c++17 -O2 main.cpp -o db -lws2_32
```

This produces `db.exe`. It does not create a database file; the demo data used by the project is held in memory while the server is running. It takes about 10–20 seconds.

> **Troubleshooting:**
> - `g++: command not found` → MSYS2 not in PATH, redo Step 1 point 5
> - `std::thread` / `std::mutex` compile errors → you are using an old MinGW compiler; switch to `C:\msys64\ucrt64\bin\g++.exe`
...
Project DB          Unified interface over all 3 (16D demo vectors)
